...
import { getSubscriptions, suspendSubscription } from '@/lib/api';
...
export default function SubscriptionsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try { const r = await getSubscriptions({ limit: 100 }); return r.data?.data ?? MOCK_SUBS; }
  ...
  const planCounts = ['STARTER', 'GROWTH', 'PRO', 'ENTERPRISE'].map((p, i) => ({
    name: p,
    value: subs.filter((s) => s.plan === p).length,
    color: PIE_COLORS[i],
  }));
  ...
  const columns: ColumnDef<Subscription>[] = [
          ...
          <div className="flex items-center gap-2">
            ...
            {s.pastDueDays && (
              <span className="text-xs text-red-400 flex items-center gap-0.5">
                <AlertTriangle className="w-3 h-3" />{s.pastDueDays}g
              </span>
            )}
          </div>
        );
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return (
          <span className={isPast ? 'text-red-400 text-xs' : 'text-xs text-[var(--text-muted)]'}>
    ...
    },
    {
      ...
      cell: ({ getValue }) => <span className="font-mono text-xs text-[var(--text-muted)]">{getValue() as string}</span>,
    },
    {
      header: '',
      id: 'actions',
        ...
        row.original.status === 'PAST_DUE' ? (
          <div className="flex gap-1">
            <Button
              size="xs"
              variant="danger"
              onClick={(e) => { e.stopPropagation(); suspendMutation.mutate(row.original.id); }}
            >
              Askıya Al
            </Button>
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-[var(--text-muted)] mt-0.5">
          {subs.length} toplam •{' '}
          {pastDue.length > 0 && <span className="text-red-400">{pastDue.length} gecikmiş ödeme</span>}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card>
          <CardHeader><CardTitle>Plan Dağılımı</CardTitle></CardHeader>
          <CardContent>
            <PieChart data={planCounts} />
          </CardContent>
        </Card>
      </div>
    </div>
