  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-primary)]">Raporlar</h1>
          <p className="text-[var(--text-secondary)] text-sm mt-1">SEO performans raporlarınızı yönetin</p>
        </div>
        <button
        ...
        >
          <Plus className="w-4 h-4" /> Rapor Oluştur
        </button>
      </div>

      {showCreate && (
        <div className="bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-xl p-5 space-y-4">
          <h2 className="font-semibold text-[var(--text-primary)]">Yeni Rapor</h2>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-[var(--text-muted)]">
          ...
          <p className="text-lg font-medium">Henüz rapor yok</p>
          <p className="text-sm mt-1">İlk raporunuzu oluşturmak için "Rapor Oluştur" düğmesine tıklayın.</p>
        </div>
      ) : (
            ...
            <div key={r.id} className="flex items-center justify-between p-4 bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--text-muted)]">{new Date(r.createdAt).toLocaleDateString('tr-TR')}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
