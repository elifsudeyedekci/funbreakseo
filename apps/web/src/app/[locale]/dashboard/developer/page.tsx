    ...
    <div className="p-6 max-w-3xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setShowModal(true)}
          ...
          <Plus className="h-4 w-4" />{t('newKeyBtn')}
        </button>
      </div>
      <div className="mb-6 rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-medium text-white mb-1">{t('docsTitle')}</p>
      ...
      {keys.length === 0 ? (
        <div className="rounded-2xl border border-white/10 p-10 text-center text-white/30 text-sm">{t('empty')}</div>
      ) : (
            ...
            <div key={k.id} className="rounded-2xl border border-white/10 bg-white/2 p-5">
              <div className="flex items-start justify-between gap-4 mb-3">
                <div>
                  ...
                  <p className="text-xs text-white/40 mt-0.5">{t('colCreated')}: {formatDate(k.createdAt)}</p>
                </div>
              ...
              <div className="flex items-center gap-2 bg-black/30 rounded-lg px-3 py-2 font-mono text-xs mb-3">
                ...
                {copiedId === k.id && <span className="text-emerald-400 text-xs">{t('copied')}</span>}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-white/10 bg-[#111118] p-6 shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-white/70 mb-1.5">{t('keyNameLabel')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-white/70 mb-2">{t('scopesLabel')}</label>
