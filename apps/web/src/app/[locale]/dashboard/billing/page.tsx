...
const STATUS_LABELS: Record<SubscriptionStatus, { label: string; color: string }> = {
  TRIALING: { label: 'Deneme', color: 'bg-blue-500/20 text-blue-400' },
  ACTIVE: { label: 'Aktif', color: 'bg-emerald-500/20 text-emerald-400' },
  PAST_DUE: { label: 'Ödeme Gecikmiş', color: 'bg-red-500/20 text-red-400' },
  SUSPENDED: { label: 'Askıya Alındı', color: 'bg-orange-500/20 text-orange-400' },
  CANCELED: { label: 'İptal Edildi', color: 'bg-white/10 text-white/40' },
};
    ...
    <div>
      <div className="flex items-center justify-between mb-1.5 text-xs">
        ...
        <span className="text-white/40">
          {used.toLocaleString('tr-TR')} / {limit >= 999999 ? 'Sınırsız' : limit.toLocaleString('tr-TR')}
        </span>
      </div>
...
export default function BillingPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold text-white">Faturalama</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/2 p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="text-xs text-white/40 mb-1">Mevcut Plan</p>
              ...
              <div className="text-right">
                <div className="text-2xl font-bold text-white">
                  {!prices || (prices.monthly === 0 && prices.yearly === 0)
                    ? 'Özel'
                    : `₺${(subscription?.billingCycle === 'YEARLY' ? prices?.yearly : prices?.monthly)?.toLocaleString('tr-TR')}`}
                </div>
                <div className="text-xs text-white/30">{subscription?.billingCycle === 'YEARLY' ? '/yıl' : '/ay'}</div>
              </div>
            </div>
            ...
            {subscription?.trialEndsAt && (
              <div className="flex items-center gap-2 rounded-lg border border-blue-500/20 bg-blue-500/10 px-3 py-2 mb-4">
                ...
                <p className="text-xs text-blue-300">
                  Deneme süresi bitiş: {formatDate(subscription.trialEndsAt)}
                </p>
              </div>
            )}
          ...
          {/* Usage quotas */}
          <div className="rounded-2xl border border-white/10 bg-white/2 p-6">
            <h2 className="text-sm font-semibold text-white mb-4">Kullanım Kotaları</h2>
            <div className="space-y-4">
              <UsageBar label="Anahtar Kelime" used={usage?.keywords?.used || 0} limit={limits?.keywords || 50} />
              <UsageBar label="Aylık Tarama" used={usage?.crawls?.used || 0} limit={limits?.monthlyCrawls || 5} />
              <UsageBar label="AI Blog" used={usage?.aiBlogs?.used || 0} limit={limits?.aiBlogsPerProject || 5} />
              <UsageBar label="GEO Sorgu" used={usage?.geoQueries?.used || 0} limit={limits?.geoQueries || 25} />
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/2 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-white">Cüzdan</h2>
              <CreditCard className="h-4 w-4 text-white/30" />
            </div>
            <div className="text-3xl font-bold text-white mb-1">
              ₺{((data?.wallet as { balance?: number })?.balance || 0).toLocaleString('tr-TR')}
            </div>
            <p className="text-xs text-white/40 mb-4">Mevcut bakiye</p>
          ...
          {/* Invoices */}
          <div className="rounded-2xl border border-white/10 bg-white/2 p-6">
            <h2 className="text-sm font-semibold text-white mb-4">Son Faturalar</h2>
            {invoices.length === 0 ? (
              <p className="text-sm text-white/30">Henüz fatura yok</p>
            ) : (
